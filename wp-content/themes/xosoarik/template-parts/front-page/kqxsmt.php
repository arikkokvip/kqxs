...
//     );
//     $results = get_lottery_results($prev_date, "xsmt");
// } elseif (isset($_GET["date"]) && $_GET["date"] != null) {
//     $results = get_lottery_results($_GET["date"], "xsmt");
// } else {
//     $results = get_lottery_results(date("d-m-Y"), "xsmt");
// }
if ($results) { ?>
    <div class="table-lottery-block">
        <table id="rsMt" class="table-lottery table table-striped">
            <caption>
                <h2>KQXS <?php echo " Miền Trung ngày " .
        ...
        <form id="numberLengthFormMt" class="formchangenumberlenght">
            <label for="sixmt"><input checked type="radio" id="sixmt" name="numberLength" value="6"> Đầy đủ</label>
            <label for="twomt"><input type="radio" id="twomt" name="numberLength" value="2"> 2 Số</label>
            <label for="threemt"><input type="radio" id="threemt" name="numberLength" value="3"> 3 Số</label>
        </form>
    </div>
...
<script>
    $(document).ready(function() {
        $('#numberLengthFormMt input[type=radio][name=numberLength]').change(function() {
            var selectedLength = parseInt(this.value); // Chuyển đổi giá trị thành số nguyên
            $('#rsMt .number-rs').each(function() {
